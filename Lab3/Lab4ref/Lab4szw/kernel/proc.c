...
PUBLIC void schedule()
{
	...
	while (1){
		...
		//判断进程是否处于阻塞状态或睡眠状态
		if (p_proc_ready->block == 0
        && p_proc_ready->sleep <= t) break;
	}
...
PUBLIC void milli_sleep(int milli_sec){
		//设置当前进程的wake_ticks
		p_proc_ready->sleep = (milli_sec * HZ / 1000) + get_ticks();
		//当前进程休眠，调度下一进程
		schedule();
}
...
PUBLIC void p_semaphore(SEMAPHORE* semaphore){
	...
	if (semaphore->value < 0) {
		p_proc_ready->block = 1;	//处于阻塞状态
		semaphore->list[semaphore->tail] = p_proc_ready;
		semaphore->tail = (semaphore->tail + 1) % NR_TASKS;	//更新临界区尾部
		schedule();		//轮询下一进程
	}
...
PUBLIC void v_semaphore(SEMAPHORE* semaphore){
	...
	if (semaphore->value <= 0) {
		...
		p->block = 0;
		semaphore->head = (semaphore->head + 1) % NR_TASKS;	//更新临界区头
	}
}

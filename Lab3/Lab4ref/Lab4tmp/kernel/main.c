...
PRIVATE void init_tasks(){
	...
	readers = 0;
	writers = 0;
	strategy = 2;//0：读写公平 1：读者优先 2：写者优先 3:生产者消费者
	delay_time = strategy == 3 ? 1 : 0;
...
void read_fair(char proc, int slices){
    P(&queue);//后来的读者，无法比先来的写者获得到queue资源
    P(&reader_mutex);
    if (++readers == 1)
        P(&writer_mutex); // 有读者正在使用，写者不可抢占
    V(&reader_mutex);
    V(&queue);

    P(&readerNum_mutex);//访问读临界区
    read_proc(slices);
    V(&readerNum_mutex);

    P(&reader_mutex);
    if (--readers == 0)
        V(&writer_mutex); // 没有读者时，可以开始写了
    V(&reader_mutex);

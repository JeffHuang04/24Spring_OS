...
const char * PleaseInput = "Please input the command!\n";
const char * ExitMore = "Input error! EXIT command should have only one exit and no other input!\n";
const char * CatShort = "Input error! CAT command requires a filename!\n";
const char * CatLong = "Input error! CAT command requires only one filename!\n";
const char * Unkown = "It is Unknown command!\n";
const char * ImgWrong = "It is a invalid FAT image\n";
...
const char * LsOpWrong = "LS: invalid option\n";
...
Image::Image(const char* path){
    ...
    int num = (int )fread(data, IMAGE_SIZE, 1, fptr);
    fclose(fptr);
    if(num != 1) {
        imageWrong();
    }//检查是否读入成功
    ...
    if(charToInt(bootDate,510,2)!=0xAA55) {
        imageWrong();
    }//检查是否引导扇区
